...
import { stockTabs, stockItems } from "@/lib/dealer-data"
import { cn } from "@/lib/utils"

export function ProductStockScreen() {
  const [tab, setTab] = useState<string>(stockTabs[0])

  const list = useMemo(
    () => (tab === "全部" ? stockItems : stockItems.filter((s) => s.series === tab)),
    [tab],
  )
            ...
            {list.map((s) => (
              <div key={s.id} className="flex items-center gap-3 rounded-2xl bg-white p-3 card-soft">
                <img
                  ...
                  alt={s.name}
                  className="h-16 w-16 shrink-0 rounded-xl bg-brand/5 object-contain"
                />
                <div className="flex flex-1 flex-col">
                  ...
                  <span className="mt-1.5 text-[13px] text-ink">
                    库存 <span className="text-lg font-black text-brand tabular-nums">{s.stock.toLocaleString()}</span> 件
                  </span>
                </div>
              </div>
            ))}
