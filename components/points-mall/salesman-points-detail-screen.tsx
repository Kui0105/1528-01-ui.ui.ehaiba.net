...
import { salesmanPointsBalance, salesmanPointsRecords } from "@/lib/dealer-data"

export function SalesmanPointsDetailScreen() {
      ...
      <div className="flex h-full flex-col bg-muted">
        ...
        <div className="brand-gradient shrink-0 text-white">
          ...
          <div className="flex flex-col gap-1 px-4 pb-6 pt-2">
            <span className="text-[12px] text-white/80">当前积分余额</span>
            <span className="text-3xl font-black tabular-nums">
              {salesmanPointsBalance} <span className="text-base font-medium">分</span>
            </span>
          </div>
        </div>
        ...
        <main className="no-scrollbar flex-1 overflow-y-auto">
          <div className="flex items-center justify-between px-4 py-2.5 text-[12px] text-muted-foreground">
            <span>
              共 {salesmanPointsRecords.length} 条 · 当前余额 <span className="font-semibold text-brand">{salesmanPointsBalance}</span> 分
            </span>
          </div>
          {salesmanPointsRecords.map((r) => (
            <div key={r.id} className="flex items-center justify-between border-b border-black/[0.05] bg-white px-4 py-3.5">
              ...
              <div className="flex flex-col items-end">
                <span className="text-lg font-black tabular-nums text-brand">+{r.amount}</span>
