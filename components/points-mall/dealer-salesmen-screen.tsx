...
export function DealerSalesmenScreen() {
  ...
  const list = useMemo(() => {
    const all = detail?.salesmen ?? []
    const kw = keyword.trim()
    if (!kw) return all
    return all.filter((s) => s.name.includes(kw) || s.phone.includes(kw) || s.region.includes(kw))
  }, [detail, keyword])

  return (
    <PhoneFrame>
      <div className="flex h-full flex-col bg-muted">
        <MobileNavBar title="业务员数据" />

        {detail && (
          <div className="shrink-0 bg-white px-4 pb-2.5 pt-2 text-[12px] text-muted-foreground">
            {detail.dealer.name} · 业务员列表
          </div>
        )}
        ...
        <main className="no-scrollbar flex-1 overflow-y-auto p-3">
          {list.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 pt-24 text-center">
              <User className="h-12 w-12 text-black/15" />
              <p className="text-[13px] text-muted-foreground">未找到匹配的业务员</p>
            </div>
          ) : (
